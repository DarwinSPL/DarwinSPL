 ...
 */
public class DwprofileOccurrence {
	
	private static interface ITokenScannerConstraint {
		public boolean mustStop(de.darwinspl.preferences.resource.dwprofile.ui.IDwprofileTokenScanner tokenScanner);
	}
	
	public final static String OCCURRENCE_ANNOTATION_ID = "de.darwinspl.preferences.resource.dwprofile.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "de.darwinspl.preferences.resource.dwprofile.ui.occurences.declaration";
	
	private final static de.darwinspl.preferences.resource.dwprofile.ui.DwprofilePositionHelper positionHelper = new de.darwinspl.preferences.resource.dwprofile.ui.DwprofilePositionHelper();
	...
	/**
	 * <p>
	 * Creates a DwprofileOccurrence object to find positions to highlight.
	 * </p>
	 * 
	 * @param textResource the text resource for location
	 * @param projectionViewer the viewer for the text
	 */
	public DwprofileOccurrence(de.darwinspl.preferences.resource.dwprofile.IDwprofileTextResource textResource, ProjectionViewer projectionViewer) {
		super();
	...
	protected int getLength(de.darwinspl.preferences.resource.dwprofile.IDwprofileLocationMap locationMap, EObject eObject) {
	...
	public void updateOccurrenceAnnotations() {
		...
		de.darwinspl.preferences.resource.dwprofile.ui.IDwprofileTokenScanner tokenScanner = scan(referencedElement, new ITokenScannerConstraint() {
	...
	protected List<String> addAnnotationsForDefinition(EObject referencedElement, String definitionText, int definitionOffset, final int caretOffset) {
		...
		if (definitionText == null) {
			...
			final List<String> names = new de.darwinspl.preferences.resource.dwprofile.analysis.DwprofileDefaultNameProvider().getNames(referencedElement);
			scan(referencedElement, new ITokenScannerConstraint() {
				...
				public boolean mustStop(de.darwinspl.preferences.resource.dwprofile.ui.IDwprofileTokenScanner tokenScanner) {
					...
					if (names.contains(text) && isContainedIn(offset, length, caretOffset)) {
						...
						addAnnotation(document, de.darwinspl.preferences.resource.dwprofile.ui.DwprofilePositionCategory.DEFINITION, text, offset, text.length());
					}
					return false;
				}
			});
			...
			addAnnotation(document, de.darwinspl.preferences.resource.dwprofile.ui.DwprofilePositionCategory.DEFINITION, definitionText, definitionOffset, definitionText.length());
	...
	protected int getIndexOf(EObject eObject, final String text) {
		de.darwinspl.preferences.resource.dwprofile.ui.IDwprofileTokenScanner tokenScanner = scan(eObject, new ITokenScannerConstraint() {
	...
	protected de.darwinspl.preferences.resource.dwprofile.ui.IDwprofileTokenScanner createTokenScanner() {
		return new de.darwinspl.preferences.resource.dwprofile.ui.DwprofileUIMetaInformation().createTokenScanner(null, null);
	}
	...
	protected void removeAnnotations() {
		removeAnnotations(de.darwinspl.preferences.resource.dwprofile.ui.DwprofileOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(de.darwinspl.preferences.resource.dwprofile.ui.DwprofileOccurrence.DECLARATION_ANNOTATION_ID);
	}
