 ...
 */
public class DwprofileBracketSet {
	...
	private ArrayList<de.darwinspl.preferences.resource.dwprofile.IDwprofileBracketPair> bracketPairs;
	private String languageID;
	
	/**
	 * Creates a new bracket set to manage bracket pairs.
	 */
	public DwprofileBracketSet() {
		super();
		this.languageID = new de.darwinspl.preferences.resource.dwprofile.mopp.DwprofileMetaInformation().getSyntaxName();
		this.bracketPairs = new ArrayList<de.darwinspl.preferences.resource.dwprofile.IDwprofileBracketPair>();
	}
	...
	public de.darwinspl.preferences.resource.dwprofile.IDwprofileBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public de.darwinspl.preferences.resource.dwprofile.IDwprofileBracketPair getBracketPair(String opening, String closing) {
		for (de.darwinspl.preferences.resource.dwprofile.IDwprofileBracketPair bracketPair : bracketPairs) {
	...
	public boolean addBracketPair(String opening, String closing, boolean closingEnabledInside, boolean closeAfterEnter) {
		...
		bracketPairs.add(new de.darwinspl.preferences.resource.dwprofile.mopp.DwprofileBracketPair(opening, closing, closingEnabledInside, closeAfterEnter));
		return true;
	}
	...
	public boolean resetBrackets(IPreferenceStore preferenceStore) {
		String bracketPairs = preferenceStore.getString(languageID + de.darwinspl.preferences.resource.dwprofile.ui.DwprofilePreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public de.darwinspl.preferences.resource.dwprofile.IDwprofileBracketPair remove(String opening, String closing) {
		for (de.darwinspl.preferences.resource.dwprofile.IDwprofileBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public void deserialize(String bracketSet) {
		bracketPairs = new ArrayList<de.darwinspl.preferences.resource.dwprofile.IDwprofileBracketPair>();
	...
	public void highlightBrackets(IDocument document, int position, int caretOffset) {
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, de.darwinspl.preferences.resource.dwprofile.ui.DwprofilePositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, de.darwinspl.preferences.resource.dwprofile.ui.DwprofilePositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
