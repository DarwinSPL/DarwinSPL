 ...
 */
public class HymanifestOccurrence {
	
	private static interface ITokenScannerConstraint {
		public boolean mustStop(eu.hyvar.mspl.manifest.resource.hymanifest.ui.IHymanifestTokenScanner tokenScanner);
	}
	
	public final static String OCCURRENCE_ANNOTATION_ID = "eu.hyvar.mspl.manifest.resource.hymanifest.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "eu.hyvar.mspl.manifest.resource.hymanifest.ui.occurences.declaration";
	
	private final static eu.hyvar.mspl.manifest.resource.hymanifest.ui.HymanifestPositionHelper positionHelper = new eu.hyvar.mspl.manifest.resource.hymanifest.ui.HymanifestPositionHelper();
	 ...
	 */
	private eu.hyvar.mspl.manifest.resource.hymanifest.IHymanifestTextResource textResource;
	...
	/**
	 * <p>
	 * Creates a HymanifestOccurrence object to find positions to highlight.
	 * </p>
	 * 
	 * @param textResource the text resource for location
	 * @param projectionViewer the viewer for the text
	 */
	public HymanifestOccurrence(eu.hyvar.mspl.manifest.resource.hymanifest.IHymanifestTextResource textResource, ProjectionViewer projectionViewer) {
		super();
	...
	protected int getLength(eu.hyvar.mspl.manifest.resource.hymanifest.IHymanifestLocationMap locationMap, EObject eObject) {
	...
	public void updateOccurrenceAnnotations() {
		...
		eu.hyvar.mspl.manifest.resource.hymanifest.ui.IHymanifestTokenScanner tokenScanner = scan(referencedElement, new ITokenScannerConstraint() {
	...
	protected List<String> addAnnotationsForDefinition(EObject referencedElement, String definitionText, int definitionOffset, final int caretOffset) {
		...
		if (definitionText == null) {
			...
			final List<String> names = new eu.hyvar.mspl.manifest.resource.hymanifest.analysis.HymanifestDefaultNameProvider().getNames(referencedElement);
			scan(referencedElement, new ITokenScannerConstraint() {
				...
				public boolean mustStop(eu.hyvar.mspl.manifest.resource.hymanifest.ui.IHymanifestTokenScanner tokenScanner) {
					...
					if (names.contains(text) && isContainedIn(offset, length, caretOffset)) {
						...
						addAnnotation(document, eu.hyvar.mspl.manifest.resource.hymanifest.ui.HymanifestPositionCategory.DEFINITION, text, offset, text.length());
					}
					return false;
				}
			});
			...
			addAnnotation(document, eu.hyvar.mspl.manifest.resource.hymanifest.ui.HymanifestPositionCategory.DEFINITION, definitionText, definitionOffset, definitionText.length());
	...
	protected int getIndexOf(EObject eObject, final String text) {
		eu.hyvar.mspl.manifest.resource.hymanifest.ui.IHymanifestTokenScanner tokenScanner = scan(eObject, new ITokenScannerConstraint() {
	...
	protected eu.hyvar.mspl.manifest.resource.hymanifest.ui.IHymanifestTokenScanner createTokenScanner() {
		return new eu.hyvar.mspl.manifest.resource.hymanifest.ui.HymanifestUIMetaInformation().createTokenScanner(null, null);
	}
	...
	protected void removeAnnotations() {
		removeAnnotations(eu.hyvar.mspl.manifest.resource.hymanifest.ui.HymanifestOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(eu.hyvar.mspl.manifest.resource.hymanifest.ui.HymanifestOccurrence.DECLARATION_ANNOTATION_ID);
	}
