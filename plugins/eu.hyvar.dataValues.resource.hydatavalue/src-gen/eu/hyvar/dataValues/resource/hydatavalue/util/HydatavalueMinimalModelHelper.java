 ...
 */
public class HydatavalueMinimalModelHelper {
	
	private final static eu.hyvar.dataValues.resource.hydatavalue.util.HydatavalueEClassUtil eClassUtil = new eu.hyvar.dataValues.resource.hydatavalue.util.HydatavalueEClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + eu.hyvar.dataValues.resource.hydatavalue.util.HydatavalueStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = eu.hyvar.dataValues.resource.hydatavalue.util.HydatavalueListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + eu.hyvar.dataValues.resource.hydatavalue.util.HydatavalueStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = eu.hyvar.dataValues.resource.hydatavalue.util.HydatavalueListUtil.castListUnchecked(value);
