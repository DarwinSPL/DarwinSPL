...
/**
 * The HydatavalueTaskItemDetector is used to find task items in text documents.
 * The current implementation searches for specific keywords to detect task items.
 * The HydatavalueTaskItemDetector is used both by the TaskItemBuilder and the
 * editor.
 */
public class HydatavalueTaskItemDetector {
	...
	public List<eu.hyvar.dataValues.resource.hydatavalue.mopp.HydatavalueTaskItem> findTaskItems(String text, int line, int charStart) {
		List<eu.hyvar.dataValues.resource.hydatavalue.mopp.HydatavalueTaskItem> foundItems = new ArrayList<eu.hyvar.dataValues.resource.hydatavalue.mopp.HydatavalueTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new eu.hyvar.dataValues.resource.hydatavalue.mopp.HydatavalueTaskItem(keyword, message, localLine, offset, end));
