...
public class HydatavalueDefaultNameProvider implements eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = eu.hyvar.dataValues.resource.hydatavalue.util.HydatavalueEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
