 ...
 */
public class HydatavaluePlugin extends Plugin {
	
	public static final String PLUGIN_ID = "eu.hyvar.dataValues.resource.hydatavalue";
	/**
	...
	private static HydatavaluePlugin plugin;
	
	public HydatavaluePlugin() {
		super();
	}
	...
	public static HydatavaluePlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, HydatavaluePlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, HydatavaluePlugin.PLUGIN_ID, message);
		}
		final HydatavaluePlugin pluginInstance = HydatavaluePlugin.getDefault();
