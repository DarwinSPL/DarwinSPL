...
public class HydatavalueSyntaxErrorMessageConverter {
	
	private String[] tokenNames;
	
	public HydatavalueSyntaxErrorMessageConverter(String[] tokenNames) {
	 ...
	 */
	public eu.hyvar.dataValues.resource.hydatavalue.mopp.HydatavalueLocalizedMessage translateLexicalError(RecognitionException e, List<RecognitionException> lexerExceptions, List<Integer> lexerExceptionPositions)  {
		String message = getMessage(e);
		return new eu.hyvar.dataValues.resource.hydatavalue.mopp.HydatavalueLocalizedMessage(message, e.charPositionInLine, e.line, lexerExceptionPositions.get(lexerExceptions.indexOf(e)), lexerExceptionPositions.get(lexerExceptions.indexOf(e)));
	}
	
	/**
	 * Translates errors thrown by the parser into human readable messages.
	 */
	public eu.hyvar.dataValues.resource.hydatavalue.mopp.HydatavalueLocalizedMessage translateParseError(RecognitionException e)  {
		String message = getMessage(e);
		
		if (e.token instanceof CommonToken) {
			CommonToken ct = (CommonToken) e.token;
			return new eu.hyvar.dataValues.resource.hydatavalue.mopp.HydatavalueLocalizedMessage(message, ct.getCharPositionInLine(), ct.getLine(), ct.getStartIndex(), ct.getStopIndex());
		} else {
			int position = 1;
			int line = 1;
			if (e.token != null) {
			...
			return new eu.hyvar.dataValues.resource.hydatavalue.mopp.HydatavalueLocalizedMessage(message, position, line, 1, 5);
		}
	}
	...
	protected String getTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = eu.hyvar.dataValues.resource.hydatavalue.util.HydatavalueStringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
