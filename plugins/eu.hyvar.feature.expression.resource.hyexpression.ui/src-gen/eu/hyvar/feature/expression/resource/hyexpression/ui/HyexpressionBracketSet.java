 ...
 */
public class HyexpressionBracketSet {
	...
	private ArrayList<eu.hyvar.feature.expression.resource.hyexpression.IHyexpressionBracketPair> bracketPairs;
	private String languageID;
	
	/**
	 * Creates a new bracket set to manage bracket pairs.
	 */
	public HyexpressionBracketSet() {
		super();
		this.languageID = new eu.hyvar.feature.expression.resource.hyexpression.mopp.HyexpressionMetaInformation().getSyntaxName();
		this.bracketPairs = new ArrayList<eu.hyvar.feature.expression.resource.hyexpression.IHyexpressionBracketPair>();
	}
	...
	public eu.hyvar.feature.expression.resource.hyexpression.IHyexpressionBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public eu.hyvar.feature.expression.resource.hyexpression.IHyexpressionBracketPair getBracketPair(String opening, String closing) {
		for (eu.hyvar.feature.expression.resource.hyexpression.IHyexpressionBracketPair bracketPair : bracketPairs) {
	...
	public boolean addBracketPair(String opening, String closing, boolean closingEnabledInside, boolean closeAfterEnter) {
		...
		bracketPairs.add(new eu.hyvar.feature.expression.resource.hyexpression.mopp.HyexpressionBracketPair(opening, closing, closingEnabledInside, closeAfterEnter));
		return true;
	}
	...
	public boolean resetBrackets(IPreferenceStore preferenceStore) {
		String bracketPairs = preferenceStore.getString(languageID + eu.hyvar.feature.expression.resource.hyexpression.ui.HyexpressionPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public eu.hyvar.feature.expression.resource.hyexpression.IHyexpressionBracketPair remove(String opening, String closing) {
		for (eu.hyvar.feature.expression.resource.hyexpression.IHyexpressionBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public void deserialize(String bracketSet) {
		bracketPairs = new ArrayList<eu.hyvar.feature.expression.resource.hyexpression.IHyexpressionBracketPair>();
	...
	public void highlightBrackets(IDocument document, int position, int caretOffset) {
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, eu.hyvar.feature.expression.resource.hyexpression.ui.HyexpressionPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, eu.hyvar.feature.expression.resource.hyexpression.ui.HyexpressionPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
