 ...
 */
public class HydatavalueOccurrence {
	
	private static interface ITokenScannerConstraint {
		public boolean mustStop(eu.hyvar.dataValues.resource.hydatavalue.ui.IHydatavalueTokenScanner tokenScanner);
	}
	
	public final static String OCCURRENCE_ANNOTATION_ID = "eu.hyvar.dataValues.resource.hydatavalue.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "eu.hyvar.dataValues.resource.hydatavalue.ui.occurences.declaration";
	
	private final static eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavaluePositionHelper positionHelper = new eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavaluePositionHelper();
	...
	/**
	 * <p>
	 * Creates a HydatavalueOccurrence object to find positions to highlight.
	 * </p>
	 * 
	 * @param textResource the text resource for location
	 * @param projectionViewer the viewer for the text
	 */
	public HydatavalueOccurrence(eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueTextResource textResource, ProjectionViewer projectionViewer) {
		super();
	...
	protected int getLength(eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueLocationMap locationMap, EObject eObject) {
	...
	public void updateOccurrenceAnnotations() {
		...
		eu.hyvar.dataValues.resource.hydatavalue.ui.IHydatavalueTokenScanner tokenScanner = scan(referencedElement, new ITokenScannerConstraint() {
	...
	protected List<String> addAnnotationsForDefinition(EObject referencedElement, String definitionText, int definitionOffset, final int caretOffset) {
		...
		if (definitionText == null) {
			...
			final List<String> names = new eu.hyvar.dataValues.resource.hydatavalue.analysis.HydatavalueDefaultNameProvider().getNames(referencedElement);
			scan(referencedElement, new ITokenScannerConstraint() {
				...
				public boolean mustStop(eu.hyvar.dataValues.resource.hydatavalue.ui.IHydatavalueTokenScanner tokenScanner) {
					...
					if (names.contains(text) && isContainedIn(offset, length, caretOffset)) {
						...
						addAnnotation(document, eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavaluePositionCategory.DEFINITION, text, offset, text.length());
					}
					return false;
				}
			});
			...
			addAnnotation(document, eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavaluePositionCategory.DEFINITION, definitionText, definitionOffset, definitionText.length());
	...
	protected int getIndexOf(EObject eObject, final String text) {
		eu.hyvar.dataValues.resource.hydatavalue.ui.IHydatavalueTokenScanner tokenScanner = scan(eObject, new ITokenScannerConstraint() {
	...
	protected eu.hyvar.dataValues.resource.hydatavalue.ui.IHydatavalueTokenScanner scan(EObject object, ITokenScannerConstraint constraint) {
		IDocument document = getSourceViewer().getDocument();
		
		eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueLocationMap locationMap = textResource.getLocationMap();
		
		eu.hyvar.dataValues.resource.hydatavalue.ui.IHydatavalueTokenScanner tokenScanner = createTokenScanner();
	...
	protected eu.hyvar.dataValues.resource.hydatavalue.ui.IHydatavalueTokenScanner createTokenScanner() {
		return new eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavalueUIMetaInformation().createTokenScanner(null, null);
	}
	...
	protected void removeAnnotations() {
		removeAnnotations(eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavalueOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavalueOccurrence.DECLARATION_ANNOTATION_ID);
	}
