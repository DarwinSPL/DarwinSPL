 ...
 */
public class HydatavalueBracketSet {
	...
	private ArrayList<eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueBracketPair> bracketPairs;
	private String languageID;
	
	/**
	 * Creates a new bracket set to manage bracket pairs.
	 */
	public HydatavalueBracketSet() {
		super();
		this.languageID = new eu.hyvar.dataValues.resource.hydatavalue.mopp.HydatavalueMetaInformation().getSyntaxName();
		this.bracketPairs = new ArrayList<eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueBracketPair>();
	}
	...
	public eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueBracketPair getBracketPair(String opening, String closing) {
		for (eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueBracketPair bracketPair : bracketPairs) {
	...
	public boolean addBracketPair(String opening, String closing, boolean closingEnabledInside, boolean closeAfterEnter) {
		...
		bracketPairs.add(new eu.hyvar.dataValues.resource.hydatavalue.mopp.HydatavalueBracketPair(opening, closing, closingEnabledInside, closeAfterEnter));
		return true;
	}
	...
	public boolean resetBrackets(IPreferenceStore preferenceStore) {
		String bracketPairs = preferenceStore.getString(languageID + eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavaluePreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueBracketPair remove(String opening, String closing) {
		for (eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public void deserialize(String bracketSet) {
		bracketPairs = new ArrayList<eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueBracketPair>();
	...
	public void highlightBrackets(IDocument document, int position, int caretOffset) {
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavaluePositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavaluePositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
