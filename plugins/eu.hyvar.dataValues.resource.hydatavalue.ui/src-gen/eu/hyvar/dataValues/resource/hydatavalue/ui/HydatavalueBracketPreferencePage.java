 ...
 * </p>
 */
public class HydatavalueBracketPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavaluePreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new eu.hyvar.dataValues.resource.hydatavalue.mopp.HydatavalueMetaInformation().getSyntaxName();
	
	private eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavalueBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public HydatavalueBracketPreferencePage() {
		super();
		
		eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueMetaInformation metaInformation = new eu.hyvar.dataValues.resource.hydatavalue.mopp.HydatavalueMetaInformation();
	...
	public void init(IWorkbench workbench) {
		...
		bracketsTmp = new eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavalueBracketSet();
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavaluePreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void addListenersToStyleButtons() {
		...
		bracketsList.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueBracketPair bracketPair = getSelectedBracketPair();
				if (bracketPair == null) {
					removeBracketButton.setEnabled(false);
					return;
				}
	...
	protected void performDefaults() {
		...
		enableCheckbox.setSelection(preferenceStore.getDefaultBoolean(eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavaluePreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		matchingBracketsColorEditor.setColorValue(PreferenceConverter.getDefaultColor(preferenceStore, BRACKETS_COLOR));
		String defaultBrackets = preferenceStore.getDefaultString(language + eu.hyvar.dataValues.resource.hydatavalue.ui.HydatavaluePreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	...
	protected eu.hyvar.dataValues.resource.hydatavalue.IHydatavalueBracketPair getSelectedBracketPair() {
