...
public class DwGroupFigure extends DwFeatureViewerReferencedFigure{
	...
	protected void createGroupTypeArc(Date date) {
		...
		HashSet<DwFeatureWrapped> features = new HashSet<DwFeatureWrapped>();
		
		DwFeatureModelWrapped model = editor.getModelWrapped();
		...
		if(features.size() > 0){
			DwFeatureWrapped firstFeature = features.iterator().next();
			DwFeatureWrapped lastFeature = firstFeature;
			Iterator<DwFeatureWrapped> it = features.iterator();
			
			while (it.hasNext()){
				DwFeatureWrapped currentFeature = it.next();
	...
	protected boolean groupTypeVisible() {
		HashSet<DwFeatureWrapped> features = group.getFeatures();
	...
	public void paintFigure(Graphics graphics) {	
		...
		DwFeatureWrapped parentFeatureWrapped = editor.getModelWrapped().getParentFeatureForGroup(group, date);
