...
public class ValidityDialog extends SelectionDialog{
	Text validSince;
	Text validUntil;
	public ValidityDialog(Shell parentShell) {
		super(parentShell);
	}
	...
	protected void createWidgets(Composite parent) {
	    ...
	    buttonSince.addSelectionListener(new SelectionListener(){
			...
			public void widgetSelected(SelectionEvent e) {
				if(element instanceof HyTemporalElement){
					DateDialog dialog = new DateDialog(getShell(), convertStringToDate(validSince.getText()));
					dialog.open();
	    ...
	    buttonUntil.addSelectionListener(new SelectionListener(){
			...
			public void widgetSelected(SelectionEvent e) {
				if(element instanceof HyTemporalElement){
					DateDialog dialog = new DateDialog(getShell(), convertStringToDate(validUntil.getText()));
					dialog.open();
					if(dialog.getReturnCode() == OK){
						Date value = dialog.getValue();
