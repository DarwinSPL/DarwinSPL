...
/**
 * The DwprofileTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * DwprofileTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class DwprofileTaskItemDetector {
	...
	public List<de.darwinspl.preferences.resource.dwprofile.mopp.DwprofileTaskItem> findTaskItems(String text, int line, int charStart) {
		List<de.darwinspl.preferences.resource.dwprofile.mopp.DwprofileTaskItem> foundItems = new ArrayList<de.darwinspl.preferences.resource.dwprofile.mopp.DwprofileTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new de.darwinspl.preferences.resource.dwprofile.mopp.DwprofileTaskItem(keyword, message, localLine, offset, end));
