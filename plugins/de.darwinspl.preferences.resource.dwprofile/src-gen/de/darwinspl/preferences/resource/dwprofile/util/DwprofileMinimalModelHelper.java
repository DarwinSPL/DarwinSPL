 ...
 */
public class DwprofileMinimalModelHelper {
	
	private final static de.darwinspl.preferences.resource.dwprofile.util.DwprofileEClassUtil eClassUtil = new de.darwinspl.preferences.resource.dwprofile.util.DwprofileEClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + de.darwinspl.preferences.resource.dwprofile.util.DwprofileStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = de.darwinspl.preferences.resource.dwprofile.util.DwprofileListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + de.darwinspl.preferences.resource.dwprofile.util.DwprofileStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = de.darwinspl.preferences.resource.dwprofile.util.DwprofileListUtil.castListUnchecked(value);
