 ...
 */
public class DwprofilePlugin extends Plugin {
	
	public static final String PLUGIN_ID = "de.darwinspl.preferences.resource.dwprofile";
	/**
	...
	private static DwprofilePlugin plugin;
	
	public DwprofilePlugin() {
		super();
	}
	...
	public static DwprofilePlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, DwprofilePlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, DwprofilePlugin.PLUGIN_ID, message);
		}
		final DwprofilePlugin pluginInstance = DwprofilePlugin.getDefault();
