...
public class DwprofileSyntaxErrorMessageConverter {
	
	private String[] tokenNames;
	
	public DwprofileSyntaxErrorMessageConverter(String[] tokenNames) {
	 ...
	 */
	public de.darwinspl.preferences.resource.dwprofile.mopp.DwprofileLocalizedMessage translateLexicalError(RecognitionException e, List<RecognitionException> lexerExceptions, List<Integer> lexerExceptionPositions)  {
		String message = getMessage(e);
		return new de.darwinspl.preferences.resource.dwprofile.mopp.DwprofileLocalizedMessage(message, e.charPositionInLine, e.line, lexerExceptionPositions.get(lexerExceptions.indexOf(e)), lexerExceptionPositions.get(lexerExceptions.indexOf(e)));
	}
	
	/**
	 * Translates errors thrown by the parser into human readable messages.
	 */
	public de.darwinspl.preferences.resource.dwprofile.mopp.DwprofileLocalizedMessage translateParseError(RecognitionException e)  {
		String message = getMessage(e);
		
		if (e.token instanceof CommonToken) {
			CommonToken ct = (CommonToken) e.token;
			return new de.darwinspl.preferences.resource.dwprofile.mopp.DwprofileLocalizedMessage(message, ct.getCharPositionInLine(), ct.getLine(), ct.getStartIndex(), ct.getStopIndex());
		} else {
			int position = 1;
			int line = 1;
			if (e.token != null) {
			...
			return new de.darwinspl.preferences.resource.dwprofile.mopp.DwprofileLocalizedMessage(message, position, line, 1, 5);
		}
	}
	...
	protected String getTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = de.darwinspl.preferences.resource.dwprofile.util.DwprofileStringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
