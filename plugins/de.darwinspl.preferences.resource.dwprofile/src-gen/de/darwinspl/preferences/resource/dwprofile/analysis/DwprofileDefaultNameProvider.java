...
public class DwprofileDefaultNameProvider implements de.darwinspl.preferences.resource.dwprofile.IDwprofileNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = de.darwinspl.preferences.resource.dwprofile.util.DwprofileEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
