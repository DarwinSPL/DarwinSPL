...
/**
 * The HycontextinformationTaskItemDetector is used to find task items in text
 * documents. The current implementation searches for specific keywords to detect
 * task items. The HycontextinformationTaskItemDetector is used both by the
 * TaskItemBuilder and the editor.
 */
public class HycontextinformationTaskItemDetector {
	...
	public List<eu.hyvar.context.resource.hycontextinformation.mopp.HycontextinformationTaskItem> findTaskItems(String text, int line, int charStart) {
		List<eu.hyvar.context.resource.hycontextinformation.mopp.HycontextinformationTaskItem> foundItems = new ArrayList<eu.hyvar.context.resource.hycontextinformation.mopp.HycontextinformationTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new eu.hyvar.context.resource.hycontextinformation.mopp.HycontextinformationTaskItem(keyword, message, localLine, offset, end));
