...
public class HycontextinformationDefaultNameProvider implements eu.hyvar.context.resource.hycontextinformation.IHycontextinformationNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = eu.hyvar.context.resource.hycontextinformation.util.HycontextinformationEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
