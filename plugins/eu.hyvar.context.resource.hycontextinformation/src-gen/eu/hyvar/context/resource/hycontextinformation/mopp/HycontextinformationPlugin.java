 ...
 */
public class HycontextinformationPlugin extends Plugin {
	
	public static final String PLUGIN_ID = "eu.hyvar.context.resource.hycontextinformation";
	/**
	...
	private static HycontextinformationPlugin plugin;
	
	public HycontextinformationPlugin() {
		super();
	}
	...
	public static HycontextinformationPlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, HycontextinformationPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, HycontextinformationPlugin.PLUGIN_ID, message);
		}
		final HycontextinformationPlugin pluginInstance = HycontextinformationPlugin.getDefault();
