 ...
 */
public class HycontextinformationMinimalModelHelper {
	
	private final static eu.hyvar.context.resource.hycontextinformation.util.HycontextinformationEClassUtil eClassUtil = new eu.hyvar.context.resource.hycontextinformation.util.HycontextinformationEClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + eu.hyvar.context.resource.hycontextinformation.util.HycontextinformationStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = eu.hyvar.context.resource.hycontextinformation.util.HycontextinformationListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + eu.hyvar.context.resource.hycontextinformation.util.HycontextinformationStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = eu.hyvar.context.resource.hycontextinformation.util.HycontextinformationListUtil.castListUnchecked(value);
