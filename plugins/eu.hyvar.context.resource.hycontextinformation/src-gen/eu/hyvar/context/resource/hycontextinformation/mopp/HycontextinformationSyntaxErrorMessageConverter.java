...
public class HycontextinformationSyntaxErrorMessageConverter {
	
	private String[] tokenNames;
	
	public HycontextinformationSyntaxErrorMessageConverter(String[] tokenNames) {
	 ...
	 */
	public eu.hyvar.context.resource.hycontextinformation.mopp.HycontextinformationLocalizedMessage translateLexicalError(RecognitionException e, List<RecognitionException> lexerExceptions, List<Integer> lexerExceptionPositions)  {
		String message = getMessage(e);
		return new eu.hyvar.context.resource.hycontextinformation.mopp.HycontextinformationLocalizedMessage(message, e.charPositionInLine, e.line, lexerExceptionPositions.get(lexerExceptions.indexOf(e)), lexerExceptionPositions.get(lexerExceptions.indexOf(e)));
	}
	
	/**
	 * Translates errors thrown by the parser into human readable messages.
	 */
	public eu.hyvar.context.resource.hycontextinformation.mopp.HycontextinformationLocalizedMessage translateParseError(RecognitionException e)  {
		String message = getMessage(e);
		
		if (e.token instanceof CommonToken) {
			CommonToken ct = (CommonToken) e.token;
			return new eu.hyvar.context.resource.hycontextinformation.mopp.HycontextinformationLocalizedMessage(message, ct.getCharPositionInLine(), ct.getLine(), ct.getStartIndex(), ct.getStopIndex());
		} else {
			int position = 1;
			int line = 1;
			if (e.token != null) {
			...
			return new eu.hyvar.context.resource.hycontextinformation.mopp.HycontextinformationLocalizedMessage(message, position, line, 1, 5);
		}
	}
	...
	protected String getTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = eu.hyvar.context.resource.hycontextinformation.util.HycontextinformationStringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
