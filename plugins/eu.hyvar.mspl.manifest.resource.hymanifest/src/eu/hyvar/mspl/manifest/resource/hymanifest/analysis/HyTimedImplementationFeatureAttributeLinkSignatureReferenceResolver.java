...
public class HyTimedImplementationFeatureAttributeLinkSignatureReferenceResolver implements eu.hyvar.mspl.manifest.resource.hymanifest.IHymanifestReferenceResolver<eu.hyvar.mspl.manifest.HyTimedImplementationFeatureAttributeLink, eu.hyvar.feature.HyFeatureAttribute> {
	...
	public void resolve(String identifier, eu.hyvar.mspl.manifest.HyTimedImplementationFeatureAttributeLink container, EReference reference, int position, boolean resolveFuzzy, final eu.hyvar.mspl.manifest.resource.hymanifest.IHymanifestReferenceResolveResult<eu.hyvar.feature.HyFeatureAttribute> result) {
		//System.out.println("UNITO DEBUG FeatureAttributeLinkSignature: identifier= "+identifier);
		...
		if(identifier!=null){
			...
			if(eobject instanceof HyTimedImplementationFeatureLink) {
				...
				HyFeature feature = hyFeatureLink.getSignature();
				HyFeatureAttribute localAttribute = container.getLocal();
				...
				while(iterator.hasNext()) {
					...
					if(featureAttribute.getClass().isInstance(localAttribute)) {
						...
						while(namesIterator.hasNext()) {
							...
							if(identifier.equals(name.getName())) {
								...
								if(HyEvolutionUtil.isWithinValidityOf(manifestInterval, enumInterval)) {
									//System.out.println("UNITO DEBUG FeatureAttributeLinkSignature: enum= "+featureAttribute);
