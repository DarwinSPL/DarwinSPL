...
public class HymanifestDefaultNameProvider implements eu.hyvar.mspl.manifest.resource.hymanifest.IHymanifestNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = eu.hyvar.mspl.manifest.resource.hymanifest.util.HymanifestEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
