 ...
 */
public class HymanifestMinimalModelHelper {
	
	private final static eu.hyvar.mspl.manifest.resource.hymanifest.util.HymanifestEClassUtil eClassUtil = new eu.hyvar.mspl.manifest.resource.hymanifest.util.HymanifestEClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + eu.hyvar.mspl.manifest.resource.hymanifest.util.HymanifestStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = eu.hyvar.mspl.manifest.resource.hymanifest.util.HymanifestListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + eu.hyvar.mspl.manifest.resource.hymanifest.util.HymanifestStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = eu.hyvar.mspl.manifest.resource.hymanifest.util.HymanifestListUtil.castListUnchecked(value);
