...
public class HymanifestSyntaxErrorMessageConverter {
	
	private String[] tokenNames;
	
	public HymanifestSyntaxErrorMessageConverter(String[] tokenNames) {
	 ...
	 */
	public eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLocalizedMessage translateLexicalError(RecognitionException e, List<RecognitionException> lexerExceptions, List<Integer> lexerExceptionPositions)  {
		String message = getMessage(e);
		return new eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLocalizedMessage(message, e.charPositionInLine, e.line, lexerExceptionPositions.get(lexerExceptions.indexOf(e)), lexerExceptionPositions.get(lexerExceptions.indexOf(e)));
	}
	
	/**
	 * Translates errors thrown by the parser into human readable messages.
	 */
	public eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLocalizedMessage translateParseError(RecognitionException e)  {
		String message = getMessage(e);
		
		if (e.token instanceof CommonToken) {
			CommonToken ct = (CommonToken) e.token;
			return new eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLocalizedMessage(message, ct.getCharPositionInLine(), ct.getLine(), ct.getStartIndex(), ct.getStopIndex());
		} else {
			int position = 1;
			int line = 1;
			if (e.token != null) {
			...
			return new eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLocalizedMessage(message, position, line, 1, 5);
		}
	}
	...
	protected String getTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = eu.hyvar.mspl.manifest.resource.hymanifest.util.HymanifestStringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
