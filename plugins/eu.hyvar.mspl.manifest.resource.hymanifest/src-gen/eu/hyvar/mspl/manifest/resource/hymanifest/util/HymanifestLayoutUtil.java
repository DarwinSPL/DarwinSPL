 ...
 */
public class HymanifestLayoutUtil {
	...
	public void transferLayoutInformationToModel(EObject element) {
		...
		for (Iterator<eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLayoutInformation> i = layoutInformationAdapter.getLayoutInformations().iterator(); i.hasNext(); ) {
			eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLayoutInformation layoutInformation = i.next();
	...
	public void transferLayoutInformationFromModel(EObject element) {
		...
		if (layoutReference != null) {
			...
			for (Iterator<EObject> i = list.iterator(); i.hasNext(); ) {
				...
				eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLayoutInformation layoutInformation = createLayoutInformation(layoutModelElement);
	...
	public eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLayoutInformation createLayoutInformation(EObject layoutInformationModelElement) {
		Object object = null;
		...
		eu.hyvar.mspl.manifest.resource.hymanifest.grammar.HymanifestSyntaxElement syntaxElement = eu.hyvar.mspl.manifest.resource.hymanifest.grammar.HymanifestGrammarInformationProvider.getSyntaxElementByID((String) layoutInformationModelElement.eGet(layoutInformationModelElement.eClass().getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME)));
		if (objectFeature != null) {
			object = layoutInformationModelElement.eGet(objectFeature);
		} else if (syntaxElement instanceof eu.hyvar.mspl.manifest.resource.hymanifest.grammar.HymanifestPlaceholder) {
			eu.hyvar.mspl.manifest.resource.hymanifest.grammar.HymanifestPlaceholder placeholder = (eu.hyvar.mspl.manifest.resource.hymanifest.grammar.HymanifestPlaceholder) syntaxElement;
			eu.hyvar.mspl.manifest.resource.hymanifest.IHymanifestTokenResolver tokenResolver = new eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestTokenResolverFactory().createTokenResolver(placeholder.getTokenName());
			eu.hyvar.mspl.manifest.resource.hymanifest.IHymanifestTokenResolveResult result = new eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestTokenResolveResult();
			tokenResolver.resolve(visibleTokenText, placeholder.getFeature(), result);
			object = result.getResolvedToken();
		}
		return new eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLayoutInformation(syntaxElement, object, startOffset, hiddenTokenText, visibleTokenText);
	}
	
	public eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLayoutInformationAdapter getLayoutInformationAdapter(EObject element) {
	...
	public void removeLayoutInformationAdapter(EObject element) {
		eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLayoutInformationAdapter existingAdapter = getLayoutInformationAdapter(element);
