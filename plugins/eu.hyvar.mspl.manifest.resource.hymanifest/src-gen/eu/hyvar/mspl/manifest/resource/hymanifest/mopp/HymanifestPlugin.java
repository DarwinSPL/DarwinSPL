 ...
 */
public class HymanifestPlugin extends Plugin {
	
	public static final String PLUGIN_ID = "eu.hyvar.mspl.manifest.resource.hymanifest";
	/**
	...
	private static HymanifestPlugin plugin;
	
	public HymanifestPlugin() {
		super();
	}
	...
	public static HymanifestPlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, HymanifestPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, HymanifestPlugin.PLUGIN_ID, message);
		}
		final HymanifestPlugin pluginInstance = HymanifestPlugin.getDefault();
