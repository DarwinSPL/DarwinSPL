...
public abstract class HymanifestANTLRParserBase extends Parser implements eu.hyvar.mspl.manifest.resource.hymanifest.IHymanifestTextParser {
	 ...
	 * result object.
	 */
	protected Collection<eu.hyvar.mspl.manifest.resource.hymanifest.IHymanifestCommand<eu.hyvar.mspl.manifest.resource.hymanifest.IHymanifestTextResource>> postParseCommands;
	 ...
	 */
	private eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestTokenResolveResult tokenResolveResult = new eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestTokenResolveResult();
	...
	public HymanifestANTLRParserBase(TokenStream input) {
		super(input);
	}
	
	public HymanifestANTLRParserBase(TokenStream input, RecognizerSharedState state) {
		super(input, state);
	}
	
	protected void retrieveLayoutInformation(EObject element, eu.hyvar.mspl.manifest.resource.hymanifest.grammar.HymanifestSyntaxElement syntaxElement, Object object, boolean ignoreTokensAfterLastVisibleToken) {
		...
		isElementToStore |= syntaxElement instanceof eu.hyvar.mspl.manifest.resource.hymanifest.grammar.HymanifestPlaceholder;
		isElementToStore |= syntaxElement instanceof eu.hyvar.mspl.manifest.resource.hymanifest.grammar.HymanifestKeyword;
		isElementToStore |= syntaxElement instanceof eu.hyvar.mspl.manifest.resource.hymanifest.grammar.HymanifestEnumerationTerminal;
		isElementToStore |= syntaxElement instanceof eu.hyvar.mspl.manifest.resource.hymanifest.grammar.HymanifestBooleanTerminal;
		if (!isElementToStore) {
			return;
		}
		eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLayoutInformationAdapter layoutInformationAdapter = getLayoutInformationAdapter(element);
		...
		layoutInformationAdapter.addLayoutInformation(new eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestLayoutInformation(syntaxElement, object, offset, hiddenTokenText.toString(), visibleTokenText.toString()));
	...
	protected <ContainerType extends EObject, ReferenceType extends EObject> void registerContextDependentProxy(final eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestContextDependentURIFragmentFactory<ContainerType, ReferenceType> factory, final ContainerType container, final EReference reference, final String id, final EObject proxy) {
		final int position;
		if (reference.isMany()) {
	...
	protected void addMapEntry(EObject element, EStructuralFeature structuralFeature, eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestDummyEObject dummy) {
		Object value = element.eGet(structuralFeature);
		Object mapKey = dummy.getValueByName("key");
		Object mapValue = dummy.getValueByName("value");
		if (value instanceof EMap<?, ?>) {
			EMap<Object, Object> valueMap = eu.hyvar.mspl.manifest.resource.hymanifest.util.HymanifestMapUtil.castToEMap(value);
	...
	protected EObject apply(EObject target, List<EObject> dummyEObjects) {
		...
		for (EObject object : dummyEObjects) {
			assert(object instanceof eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestDummyEObject);
			eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestDummyEObject dummy = (eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestDummyEObject) object;
	...
	protected eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestTokenResolveResult getFreshTokenResolveResult() {
	...
	protected eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestReferenceResolverSwitch getReferenceResolverSwitch() {
		eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestReferenceResolverSwitch resolverSwitch = (eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestReferenceResolverSwitch) metaInformation.getReferenceResolverSwitch();
	...
	public boolean isLayoutInformationRecordingEnabled() {
		...
		Object value = options.get(eu.hyvar.mspl.manifest.resource.hymanifest.IHymanifestOptions.DISABLE_LAYOUT_INFORMATION_RECORDING);
	...
	public boolean isLocationMapEnabled() {
		...
		Object value = options.get(eu.hyvar.mspl.manifest.resource.hymanifest.IHymanifestOptions.DISABLE_LOCATION_MAP);
		return value == null || Boolean.FALSE.equals(value);
	}
