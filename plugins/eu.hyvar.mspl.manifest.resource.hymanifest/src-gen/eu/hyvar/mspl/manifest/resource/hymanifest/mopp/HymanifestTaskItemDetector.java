...
/**
 * The HymanifestTaskItemDetector is used to find task items in text documents.
 * The current implementation searches for specific keywords to detect task items.
 * The HymanifestTaskItemDetector is used both by the TaskItemBuilder and the
 * editor.
 */
public class HymanifestTaskItemDetector {
	...
	public List<eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestTaskItem> findTaskItems(String text, int line, int charStart) {
		List<eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestTaskItem> foundItems = new ArrayList<eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new eu.hyvar.mspl.manifest.resource.hymanifest.mopp.HymanifestTaskItem(keyword, message, localLine, offset, end));
