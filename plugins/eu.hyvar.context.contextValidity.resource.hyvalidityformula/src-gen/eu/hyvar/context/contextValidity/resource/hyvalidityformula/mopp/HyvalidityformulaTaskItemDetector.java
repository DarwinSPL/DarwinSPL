...
/**
 * The HyvalidityformulaTaskItemDetector is used to find task items in text
 * documents. The current implementation searches for specific keywords to detect
 * task items. The HyvalidityformulaTaskItemDetector is used both by the
 * TaskItemBuilder and the editor.
 */
public class HyvalidityformulaTaskItemDetector {
	...
	public List<eu.hyvar.context.contextValidity.resource.hyvalidityformula.mopp.HyvalidityformulaTaskItem> findTaskItems(String text, int line, int charStart) {
		List<eu.hyvar.context.contextValidity.resource.hyvalidityformula.mopp.HyvalidityformulaTaskItem> foundItems = new ArrayList<eu.hyvar.context.contextValidity.resource.hyvalidityformula.mopp.HyvalidityformulaTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new eu.hyvar.context.contextValidity.resource.hyvalidityformula.mopp.HyvalidityformulaTaskItem(keyword, message, localLine, offset, end));
