...
public class HyvalidityformulaDefaultNameProvider implements eu.hyvar.context.contextValidity.resource.hyvalidityformula.IHyvalidityformulaNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = eu.hyvar.context.contextValidity.resource.hyvalidityformula.util.HyvalidityformulaEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
