 ...
 */
public class HyvalidityformulaPlugin extends Plugin {
	
	public static final String PLUGIN_ID = "eu.hyvar.context.contextValidity.resource.hyvalidityformula";
	/**
	...
	private static HyvalidityformulaPlugin plugin;
	
	public HyvalidityformulaPlugin() {
		super();
	}
	...
	public static HyvalidityformulaPlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, HyvalidityformulaPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, HyvalidityformulaPlugin.PLUGIN_ID, message);
		}
		final HyvalidityformulaPlugin pluginInstance = HyvalidityformulaPlugin.getDefault();
