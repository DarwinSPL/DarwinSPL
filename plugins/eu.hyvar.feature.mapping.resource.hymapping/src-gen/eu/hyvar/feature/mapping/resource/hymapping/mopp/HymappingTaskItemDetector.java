...
/**
 * The HymappingTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * HymappingTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class HymappingTaskItemDetector {
	...
	public List<eu.hyvar.feature.mapping.resource.hymapping.mopp.HymappingTaskItem> findTaskItems(String text, int line, int charStart) {
		List<eu.hyvar.feature.mapping.resource.hymapping.mopp.HymappingTaskItem> foundItems = new ArrayList<eu.hyvar.feature.mapping.resource.hymapping.mopp.HymappingTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new eu.hyvar.feature.mapping.resource.hymapping.mopp.HymappingTaskItem(keyword, message, localLine, offset, end));
