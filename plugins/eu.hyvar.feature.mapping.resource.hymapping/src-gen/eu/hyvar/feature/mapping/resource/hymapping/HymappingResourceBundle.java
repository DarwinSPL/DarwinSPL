 ...
 */
public class HymappingResourceBundle {
	
	/**
	 * The name of the main task that is shown in the progress view when the builders
	 * are running.
	 */
	public static String BUILDER_ADAPTER_TASK_NAME = "Building hymapping file";
	...
	static {
		try {
			ResourceBundle bundle = ResourceBundle.getBundle(HymappingResourceBundle.class.getName(), Locale.getDefault());
			if (bundle != null) {
				Set<String> keys = bundle.keySet();
				for (String key : keys) {
					String value = bundle.getString(key);
					try {
						Field field = HymappingResourceBundle.class.getDeclaredField(key.toUpperCase());
