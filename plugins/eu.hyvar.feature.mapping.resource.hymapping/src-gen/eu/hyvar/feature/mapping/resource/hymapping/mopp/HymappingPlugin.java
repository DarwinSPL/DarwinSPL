 ...
 */
public class HymappingPlugin extends Plugin {
	
	public static final String PLUGIN_ID = "eu.hyvar.feature.mapping.resource.hymapping";
	/**
	...
	private static HymappingPlugin plugin;
	
	public HymappingPlugin() {
		super();
	}
	...
	public static HymappingPlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, HymappingPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, HymappingPlugin.PLUGIN_ID, message);
		}
		final HymappingPlugin pluginInstance = HymappingPlugin.getDefault();
