...
public class HymappingDefaultNameProvider implements eu.hyvar.feature.mapping.resource.hymapping.IHymappingNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = eu.hyvar.feature.mapping.resource.hymapping.util.HymappingEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
