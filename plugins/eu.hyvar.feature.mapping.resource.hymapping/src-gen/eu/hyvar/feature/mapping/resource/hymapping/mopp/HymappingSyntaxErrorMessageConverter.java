...
public class HymappingSyntaxErrorMessageConverter {
	
	private String[] tokenNames;
	
	public HymappingSyntaxErrorMessageConverter(String[] tokenNames) {
	 ...
	 */
	public eu.hyvar.feature.mapping.resource.hymapping.mopp.HymappingLocalizedMessage translateLexicalError(RecognitionException e, List<RecognitionException> lexerExceptions, List<Integer> lexerExceptionPositions)  {
		String message = getMessage(e);
		return new eu.hyvar.feature.mapping.resource.hymapping.mopp.HymappingLocalizedMessage(message, e.charPositionInLine, e.line, lexerExceptionPositions.get(lexerExceptions.indexOf(e)), lexerExceptionPositions.get(lexerExceptions.indexOf(e)));
	}
	
	/**
	 * Translates errors thrown by the parser into human readable messages.
	 */
	public eu.hyvar.feature.mapping.resource.hymapping.mopp.HymappingLocalizedMessage translateParseError(RecognitionException e)  {
		String message = getMessage(e);
		
		if (e.token instanceof CommonToken) {
			CommonToken ct = (CommonToken) e.token;
			return new eu.hyvar.feature.mapping.resource.hymapping.mopp.HymappingLocalizedMessage(message, ct.getCharPositionInLine(), ct.getLine(), ct.getStartIndex(), ct.getStopIndex());
		} else {
			int position = 1;
			int line = 1;
			if (e.token != null) {
			...
			return new eu.hyvar.feature.mapping.resource.hymapping.mopp.HymappingLocalizedMessage(message, position, line, 1, 5);
		}
	}
	...
	protected String getTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = eu.hyvar.feature.mapping.resource.hymapping.util.HymappingStringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
