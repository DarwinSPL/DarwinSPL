...
public class DwRESTServerSelectExtendedDialog extends DwRESTServerSelectDialog {
	...
	public void create() {
		...
		setMessage(
				"Change the address of the rest server for reconfiguration the selected feature model. And adjust options concerning evolution.",
	...
	protected void savePluginSettings() {
		...
		pref.putBoolean("isEvoAware", isEvolutionAwareAnalysis);
		pref.putLong("startDate", startDate.getTime());
		pref.putLong("endDate", endDate.getTime());
		pref.putInt("EvoAwareAnalysisType", evolutionAwareAnalysisType.ordinal());
		
		super.savePluginSettings();
	}
	...
	protected void loadPluginSettings() {
		...
		this.isEvolutionAwareAnalysis = pref.getBoolean("isEvoAware", false);
		this.startDate = new Date(pref.getLong("startDate", Long.MIN_VALUE));
		this.endDate = new Date(pref.getLong("endDate", Long.MAX_VALUE));
		this.evolutionAwareAnalysisType = TypeOfEvolutionAwareAnalysis.values()[pref.getInt("EvoAwareAnalysisType", 0)];
