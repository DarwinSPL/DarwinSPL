...
public class AnomalyTableView<T extends DwAnomaly> extends TableViewer {
	...
	public AnomalyTableView(DwAnomalyView anomalyView, Composite parent, int style, List<T> falseOptionalAnomalies,
			String[] titles) {
		super(parent, style);
	...
	private void createColumnsFalseOptionalAnomalies(final Composite parent, String[] titles) {
		...
		// second column is for the name of the effected features or the
		// contextual information values
		...
		col.setLabelProvider(new ColumnLabelProvider() {
			...
			@Override
			public void update(ViewerCell cell) {
				if (cell.getItem().getData() instanceof DwDeadFeatureAnomaly
						|| cell.getItem().getData() instanceof DwFalseOptionalFeatureAnomaly) {
					TableItem item = (TableItem) cell.getItem();
					Button button = new Button((Composite) cell.getViewerRow().getControl(), SWT.NONE);
					button.setData(item);
