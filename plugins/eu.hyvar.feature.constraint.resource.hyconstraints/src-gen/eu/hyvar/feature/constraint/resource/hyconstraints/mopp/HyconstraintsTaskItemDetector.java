...
/**
 * The HyconstraintsTaskItemDetector is used to find task items in text documents.
 * The current implementation searches for specific keywords to detect task items.
 * The HyconstraintsTaskItemDetector is used both by the TaskItemBuilder and the
 * editor.
 */
public class HyconstraintsTaskItemDetector {
	...
	public List<eu.hyvar.feature.constraint.resource.hyconstraints.mopp.HyconstraintsTaskItem> findTaskItems(String text, int line, int charStart) {
		List<eu.hyvar.feature.constraint.resource.hyconstraints.mopp.HyconstraintsTaskItem> foundItems = new ArrayList<eu.hyvar.feature.constraint.resource.hyconstraints.mopp.HyconstraintsTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new eu.hyvar.feature.constraint.resource.hyconstraints.mopp.HyconstraintsTaskItem(keyword, message, localLine, offset, end));
