 ...
 */
public class HyconstraintsMinimalModelHelper {
	
	private final static eu.hyvar.feature.constraint.resource.hyconstraints.util.HyconstraintsEClassUtil eClassUtil = new eu.hyvar.feature.constraint.resource.hyconstraints.util.HyconstraintsEClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + eu.hyvar.feature.constraint.resource.hyconstraints.util.HyconstraintsStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = eu.hyvar.feature.constraint.resource.hyconstraints.util.HyconstraintsListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + eu.hyvar.feature.constraint.resource.hyconstraints.util.HyconstraintsStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = eu.hyvar.feature.constraint.resource.hyconstraints.util.HyconstraintsListUtil.castListUnchecked(value);
