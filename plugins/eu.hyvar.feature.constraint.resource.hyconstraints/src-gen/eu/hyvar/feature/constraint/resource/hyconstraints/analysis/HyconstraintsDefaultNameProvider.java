...
public class HyconstraintsDefaultNameProvider implements eu.hyvar.feature.constraint.resource.hyconstraints.IHyconstraintsNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = eu.hyvar.feature.constraint.resource.hyconstraints.util.HyconstraintsEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
