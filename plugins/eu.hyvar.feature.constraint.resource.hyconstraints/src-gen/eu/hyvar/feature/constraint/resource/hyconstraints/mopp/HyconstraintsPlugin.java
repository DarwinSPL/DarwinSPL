 ...
 */
public class HyconstraintsPlugin extends Plugin {
	
	public static final String PLUGIN_ID = "eu.hyvar.feature.constraint.resource.hyconstraints";
	/**
	...
	private static HyconstraintsPlugin plugin;
	
	public HyconstraintsPlugin() {
		super();
	}
	...
	public static HyconstraintsPlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, HyconstraintsPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, HyconstraintsPlugin.PLUGIN_ID, message);
		}
		final HyconstraintsPlugin pluginInstance = HyconstraintsPlugin.getDefault();
