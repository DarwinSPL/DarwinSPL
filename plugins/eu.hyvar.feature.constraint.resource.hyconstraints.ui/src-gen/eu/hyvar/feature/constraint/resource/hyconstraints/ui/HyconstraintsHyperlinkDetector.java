 ...
 * hovers, is a proxy.
 */
public class HyconstraintsHyperlinkDetector implements IHyperlinkDetector {
	 ...
	 */
	public HyconstraintsHyperlinkDetector(Resource resource) {
		textResource = (eu.hyvar.feature.constraint.resource.hyconstraints.IHyconstraintsTextResource) resource;
	}
		...
		for (EObject eObject : elementsAtOffset) {
			if (eObject.eIsProxy()) {
				...
				if (resolvedEObject.eResource() != null) {
					IHyperlink hyperlink = new eu.hyvar.feature.constraint.resource.hyconstraints.ui.HyconstraintsHyperlink(new Region(offset, length), resolvedEObject, text);
