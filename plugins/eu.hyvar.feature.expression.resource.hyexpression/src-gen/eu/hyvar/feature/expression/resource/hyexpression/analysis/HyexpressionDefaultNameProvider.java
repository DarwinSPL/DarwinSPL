...
public class HyexpressionDefaultNameProvider implements eu.hyvar.feature.expression.resource.hyexpression.IHyexpressionNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = eu.hyvar.feature.expression.resource.hyexpression.util.HyexpressionEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
