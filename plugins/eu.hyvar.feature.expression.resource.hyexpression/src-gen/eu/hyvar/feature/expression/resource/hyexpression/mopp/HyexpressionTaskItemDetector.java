...
/**
 * The HyexpressionTaskItemDetector is used to find task items in text documents.
 * The current implementation searches for specific keywords to detect task items.
 * The HyexpressionTaskItemDetector is used both by the TaskItemBuilder and the
 * editor.
 */
public class HyexpressionTaskItemDetector {
	...
	public List<eu.hyvar.feature.expression.resource.hyexpression.mopp.HyexpressionTaskItem> findTaskItems(String text, int line, int charStart) {
		List<eu.hyvar.feature.expression.resource.hyexpression.mopp.HyexpressionTaskItem> foundItems = new ArrayList<eu.hyvar.feature.expression.resource.hyexpression.mopp.HyexpressionTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new eu.hyvar.feature.expression.resource.hyexpression.mopp.HyexpressionTaskItem(keyword, message, localLine, offset, end));
