 ...
 */
public class HyexpressionMinimalModelHelper {
	
	private final static eu.hyvar.feature.expression.resource.hyexpression.util.HyexpressionEClassUtil eClassUtil = new eu.hyvar.feature.expression.resource.hyexpression.util.HyexpressionEClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + eu.hyvar.feature.expression.resource.hyexpression.util.HyexpressionStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = eu.hyvar.feature.expression.resource.hyexpression.util.HyexpressionListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + eu.hyvar.feature.expression.resource.hyexpression.util.HyexpressionStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = eu.hyvar.feature.expression.resource.hyexpression.util.HyexpressionListUtil.castListUnchecked(value);
