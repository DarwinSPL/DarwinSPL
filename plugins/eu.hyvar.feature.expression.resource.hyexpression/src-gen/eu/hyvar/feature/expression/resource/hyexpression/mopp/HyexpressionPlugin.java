 ...
 */
public class HyexpressionPlugin extends Plugin {
	
	public static final String PLUGIN_ID = "eu.hyvar.feature.expression.resource.hyexpression";
	/**
	...
	private static HyexpressionPlugin plugin;
	
	public HyexpressionPlugin() {
		super();
	}
	...
	public static HyexpressionPlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, HyexpressionPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, HyexpressionPlugin.PLUGIN_ID, message);
		}
		final HyexpressionPlugin pluginInstance = HyexpressionPlugin.getDefault();
