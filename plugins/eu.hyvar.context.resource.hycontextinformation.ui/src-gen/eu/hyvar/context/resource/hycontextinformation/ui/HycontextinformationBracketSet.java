 ...
 */
public class HycontextinformationBracketSet {
	...
	private ArrayList<eu.hyvar.context.resource.hycontextinformation.IHycontextinformationBracketPair> bracketPairs;
	private String languageID;
	
	/**
	 * Creates a new bracket set to manage bracket pairs.
	 */
	public HycontextinformationBracketSet() {
		super();
		...
		this.bracketPairs = new ArrayList<eu.hyvar.context.resource.hycontextinformation.IHycontextinformationBracketPair>();
	}
	...
	public eu.hyvar.context.resource.hycontextinformation.IHycontextinformationBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public eu.hyvar.context.resource.hycontextinformation.IHycontextinformationBracketPair getBracketPair(String opening, String closing) {
		for (eu.hyvar.context.resource.hycontextinformation.IHycontextinformationBracketPair bracketPair : bracketPairs) {
	...
	public boolean addBracketPair(String opening, String closing, boolean closingEnabledInside, boolean closeAfterEnter) {
		...
		bracketPairs.add(new eu.hyvar.context.resource.hycontextinformation.mopp.HycontextinformationBracketPair(opening, closing, closingEnabledInside, closeAfterEnter));
		return true;
	}
	...
	public boolean resetBrackets(IPreferenceStore preferenceStore) {
		String bracketPairs = preferenceStore.getString(languageID + eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public eu.hyvar.context.resource.hycontextinformation.IHycontextinformationBracketPair remove(String opening, String closing) {
		for (eu.hyvar.context.resource.hycontextinformation.IHycontextinformationBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public void deserialize(String bracketSet) {
		bracketPairs = new ArrayList<eu.hyvar.context.resource.hycontextinformation.IHycontextinformationBracketPair>();
	...
	public void highlightBrackets(IDocument document, int position, int caretOffset) {
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
