 ...
 */
public class HycontextinformationOccurrence {
	
	private static interface ITokenScannerConstraint {
		public boolean mustStop(eu.hyvar.context.resource.hycontextinformation.ui.IHycontextinformationTokenScanner tokenScanner);
	}
	
	public final static String OCCURRENCE_ANNOTATION_ID = "eu.hyvar.context.resource.hycontextinformation.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "eu.hyvar.context.resource.hycontextinformation.ui.occurences.declaration";
	
	private final static eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPositionHelper positionHelper = new eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPositionHelper();
	 ...
	 */
	private eu.hyvar.context.resource.hycontextinformation.IHycontextinformationTextResource textResource;
	...
	/**
	 * <p>
	 * Creates a HycontextinformationOccurrence object to find positions to highlight.
	 * </p>
	 * 
	 * @param textResource the text resource for location
	 * @param projectionViewer the viewer for the text
	 */
	public HycontextinformationOccurrence(eu.hyvar.context.resource.hycontextinformation.IHycontextinformationTextResource textResource, ProjectionViewer projectionViewer) {
		super();
	...
	protected int getLength(eu.hyvar.context.resource.hycontextinformation.IHycontextinformationLocationMap locationMap, EObject eObject) {
	...
	public void updateOccurrenceAnnotations() {
		...
		eu.hyvar.context.resource.hycontextinformation.ui.IHycontextinformationTokenScanner tokenScanner = scan(referencedElement, new ITokenScannerConstraint() {
	...
	protected void addAnnotationsForReferences(EObject referencedElement, List<String> matchingNames) {
		...
		for (Setting setting : referencingObjects) {
			...
			for (String name : matchingNames) {
				...
				if (index > 0) {
					addAnnotation(document, eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPositionCategory.PROXY, name, index, name.length());
				}
			}
		}
	}
	...
	protected List<String> addAnnotationsForDefinition(EObject referencedElement, String definitionText, int definitionOffset, final int caretOffset) {
		...
		if (definitionText == null) {
			...
			final List<String> names = new eu.hyvar.context.resource.hycontextinformation.analysis.HycontextinformationDefaultNameProvider().getNames(referencedElement);
			scan(referencedElement, new ITokenScannerConstraint() {
				...
				public boolean mustStop(eu.hyvar.context.resource.hycontextinformation.ui.IHycontextinformationTokenScanner tokenScanner) {
					...
					if (names.contains(text) && isContainedIn(offset, length, caretOffset)) {
						...
						addAnnotation(document, eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPositionCategory.DEFINITION, text, offset, text.length());
					}
					return false;
				}
			});
			...
			addAnnotation(document, eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPositionCategory.DEFINITION, definitionText, definitionOffset, definitionText.length());
	...
	protected int getIndexOf(EObject eObject, final String text) {
		eu.hyvar.context.resource.hycontextinformation.ui.IHycontextinformationTokenScanner tokenScanner = scan(eObject, new ITokenScannerConstraint() {
	...
	protected eu.hyvar.context.resource.hycontextinformation.ui.IHycontextinformationTokenScanner scan(EObject object, ITokenScannerConstraint constraint) {
		IDocument document = getSourceViewer().getDocument();
		
		eu.hyvar.context.resource.hycontextinformation.IHycontextinformationLocationMap locationMap = textResource.getLocationMap();
		
		eu.hyvar.context.resource.hycontextinformation.ui.IHycontextinformationTokenScanner tokenScanner = createTokenScanner();
	...
	protected eu.hyvar.context.resource.hycontextinformation.ui.IHycontextinformationTokenScanner createTokenScanner() {
		return new eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationUIMetaInformation().createTokenScanner(null, null);
	}
	
	protected void addAnnotation(IDocument document, eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPositionCategory type, String text, int offset, int length) {
		...
		if (type == eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPositionCategory.DEFINITION) {
	...
	protected void removeAnnotations() {
		removeAnnotations(eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationOccurrence.DECLARATION_ANNOTATION_ID);
	}
