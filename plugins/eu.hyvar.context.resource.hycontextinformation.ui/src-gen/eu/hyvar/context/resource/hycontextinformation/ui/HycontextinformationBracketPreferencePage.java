 ...
 * </p>
 */
public class HycontextinformationBracketPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new eu.hyvar.context.resource.hycontextinformation.mopp.HycontextinformationMetaInformation().getSyntaxName();
	
	private eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public HycontextinformationBracketPreferencePage() {
		super();
		
		eu.hyvar.context.resource.hycontextinformation.IHycontextinformationMetaInformation metaInformation = new eu.hyvar.context.resource.hycontextinformation.mopp.HycontextinformationMetaInformation();
	...
	public void init(IWorkbench workbench) {
		...
		bracketsTmp = new eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationBracketSet();
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.deserialize(getPreferenceStore().getString(language + eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
	...
	private void addListenersToStyleButtons() {
		...
		bracketsList.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				eu.hyvar.context.resource.hycontextinformation.IHycontextinformationBracketPair bracketPair = getSelectedBracketPair();
				if (bracketPair == null) {
					removeBracketButton.setEnabled(false);
					return;
				}
	...
	protected void performDefaults() {
		...
		enableCheckbox.setSelection(preferenceStore.getDefaultBoolean(eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		matchingBracketsColorEditor.setColorValue(PreferenceConverter.getDefaultColor(preferenceStore, BRACKETS_COLOR));
		String defaultBrackets = preferenceStore.getDefaultString(language + eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	...
	private void updateActiveEditor() {
		...
		preferenceStore.setValue(eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		preferenceStore.setValue(language + eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		IWorkbench workbench = org.eclipse.ui.PlatformUI.getWorkbench();
		IEditorPart editor = workbench.getActiveWorkbenchWindow().getActivePage().getActiveEditor();
		if (editor != null && editor instanceof eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationEditor) {
			((eu.hyvar.context.resource.hycontextinformation.ui.HycontextinformationEditor) editor).invalidateTextRepresentation();
		}
	}
	
	protected eu.hyvar.context.resource.hycontextinformation.IHycontextinformationBracketPair getSelectedBracketPair() {
